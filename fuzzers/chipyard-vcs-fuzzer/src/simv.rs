...
impl<'a> CommandConfigurator for SimvCommandConfigurator<'a> {
    ...
    fn spawn_child<I: Input + HasTargetBytes>(&mut self, input: &I) -> Result<Child, Error> {
        ...
        forged_cmd_args.push_str(" +permissive-off ./testcase.elf");
        ...
        // 3. spawn simv
        Ok(Command::new("./simv-chipyard.harness-RocketConfig")
            .args(&args_vec)
            .stdin(Stdio::null())
            .stdout(hw_execution_log)
            .stderr(hw_execution_err)
            .spawn()
...
impl<'a> SimvCommandConfigurator<'a> {
    ...
    pub fn new_from_config_file(config_filename: &'static str, workdir: &str, testcase_buf: &'a mut [u8], shm_id: &'static str, seed: u32) -> SimvCommandConfigurator<'a> {
        ...
        let src_s = vec![
            format!("{}/simv-chipyard.harness-RocketConfig.daidir", vcs_home),
            format!("{}/simv-chipyard.harness-RocketConfig", vcs_home),
            format!("{}/iram.elf", vcs_home)];
 
        let dst_s = vec![
            format!("{}/simv-chipyard.harness-RocketConfig.daidir", workdir),
            format!("{}/simv-chipyard.harness-RocketConfig", workdir),
